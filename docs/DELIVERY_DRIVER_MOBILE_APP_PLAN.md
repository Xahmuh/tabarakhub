...
>
> **QA update (2026-06-16):** Production route smoke passed for `/` and `/delivery`, production JS contains the driver UI/RPC integration, and the local Expo web app runs at `http://localhost:8091`. Real driver login E2E is still blocked until an Admin-dashboard session creates a Driver login and links it to an active `delivery_drivers` row. See `docs/DRIVER_APP_QA_RESULTS.md`.
>
