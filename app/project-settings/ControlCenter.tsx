...
import { BahrainLicensePlate, FleetMotorcyclePlate, FleetMotorcyclesBar } from '../delivery/components/BahrainLicensePlate';
...
const PILLARS: PillarConfig[] = [
  ...
  },
  {
    ...
    description: 'Pharmacy branch roster, GPS boundaries, and delivery service rings.',
    icon: Building2,
    tone: 'emerald',
    tabs: [
      {
        id: 'branches',
        label: 'Branch Roster & Fleet',
        labelAr: 'الفروع والأسطول',
        description: 'Pharmacy branches, GPS radius, and staff assignment',
        icon: Store
      },
      {
        id: 'delivery-zones',
    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      ...
      {isLoading ? (
        ...
        <main className="space-y-6">
          ...
          {/* TAB 2.1: BRANCH ROSTER & FLEET */}
          {activeTab === 'branches' && (
            <div className="space-y-5">
              <section className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="text-base font-black tracking-tight text-slate-900">
                    Pharmacy Branch Roster & Staff Fleet
                  </h3>
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    Configure GPS origin points, duty radius, and assign pharmacists and delivery drivers.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                  <div className="relative w-full sm:w-64">
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                {filteredBranches.map(branch => {
                  ...
                  return (
                    <article key={branch.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-base font-black text-slate-950">{branch.name}</h4>
                          ...
                          <p className="mt-1 text-xs font-medium text-slate-500">
                            Manager: {branch.branchManagerName || 'Not assigned'}
                          </p>
                          <p className="mt-0.5 text-[11px] font-bold text-slate-400">
                            GPS: {branch.lat && branch.lng ? `${branch.lat.toFixed(4)}, ${branch.lng.toFixed(4)}` : 'Not set'} • Radius: {branch.dutyRadiusM || DEFAULT_BRANCH_DUTY_RADIUS_M}m
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="btn-secondary text-[10px] uppercase tracking-widest"
                            title="Assign Pharmacists & Drivers"
                          >
                            <Users className="h-3.5 w-3.5" />
                            Staff ({assignment.pharmacistIds.length + assignment.driverIds.length})
                          </button>
                          <button
                            onClick={() => {
                      ...
                      {/* Fleet Motorcycle Plates & Staff matching Actions & Renewable template */}
                      {(assignedDrivers.length > 0 || assignedPharmacists.length > 0) && (
                        <div className="mt-4 flex flex-col gap-2.5 border-t border-slate-100 pt-3">
                          {assignedDrivers.length > 0 && (
                            <div className="flex flex-wrap items-center gap-2">
                              <div className="flex items-center gap-1.5">
                                <span className="text-sm leading-none" role="img" aria-label="helmet">⛑️</span>
                          ...
                          {assignedPharmacists.length > 0 && (
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-[11px] font-black uppercase tracking-wider text-slate-400">Pharmacists:</span>
                              {assignedPharmacists.map(p => (
                                <div key={p.id} className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2 py-0.5 text-[11px] font-bold text-slate-700 shadow-2xs">
