...
import { DriverAvatar } from '../../shared/EmployeeRoleAvatar';
...
/**
 * Exact Fleet Vehicle Plate component matching the Actions & Renewable template.
 * Features the clean badge with the red vector flat-2 driver avatar icon (with helmet) and bold plate number.
 */
export const FleetVehiclePlate: React.FC<{
  plateNumber?: string | number | null;
  size?: 'xs' | 'sm' | 'md' | 'lg';
  className?: string;
}> = ({
  ...
  return (
    <div
      className={`inline-flex items-center border border-slate-200 bg-white font-black text-slate-800 shadow-2xs select-none transition hover:border-brand/40 hover:bg-slate-50 ${sizeClasses} ${className}`}
      style={{
        fontFamily: '"Segoe UI", Tahoma, Geneva, Verdana, sans-serif'
      }}
    >
      {/* Red Flat-2 Driver Avatar Icon with Helmet */}
      <DriverAvatar size={avatarSize} className="shrink-0 shadow-none" />

      {/* Bold Plate Number */}
      <span
        className="tracking-wider leading-none tabular-nums font-black text-slate-900"
        style={{
...
export const FleetMotorcyclePlate = FleetVehiclePlate;

/**
 * Full "FLEET VEHICLES:" Row Header & Badges matching the exact Actions & Renewable template.
 */
export const FleetVehiclesBar: React.FC<{
  ...
  return (
    <div className={`inline-flex flex-wrap items-center gap-2.5 ${className}`}>
      {/* Red Flat-2 Driver Avatar Icon + FLEET VEHICLES: Label */}
      ...
      <div className="flex flex-wrap items-center gap-2">
        {validPlates.map((plate, index) => (
          <FleetVehiclePlate key={`${plate}-${index}`} plateNumber={plate} size={size} />
        ))}
      </div>
    </div>
  );
...
// Backward-compatibility alias
export const FleetMotorcyclesBar = FleetVehiclesBar;
...
export const BahrainLicensePlate: React.FC<BahrainLicensePlateProps> = ({
  ...
  if (variant === 'fleet-pill') {
    return <FleetVehiclePlate plateNumber={plateNumber} size={size} className={className} />;
  }
