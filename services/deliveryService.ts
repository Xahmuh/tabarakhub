...
export const deliveryService = {
  ...
  blocks: {
    list: async (includeInactive = false): Promise<DeliveryBlock[]> => {
      ...
      return all.map(b => ({
        blockNumber: b.block_number, areaName: b.area_name, governorate: b.governorate, isActive: b.is_active
      }));
    },
    resolve: async (blockNumber: string): Promise<DeliveryBlock | null> => {
      ...
      return { blockNumber: data.block_number, areaName: data.area_name, governorate: data.governorate, isActive: data.is_active };
    },
